...
const Calendar = (props) => {
    const { initialDate, current, theme, markedDates, minDate, maxDate, allowSelectionOutOfRange, onDayPress,selectWeekFunc , onDayLongPress, onMonthChange, onVisibleMonthsChange, disableMonthChange, enableSwipeMonths, hideExtraDays, firstDay, showSixWeeks, displayLoadingIndicator, customHeader, headerStyle, accessibilityElementsHidden, importantForAccessibility, testID, style: propsStyle } = props;
    ...
    const renderWeek = (days, id) => {
        ...
        return (<View style={[style.current.week,{alignItems: 'center'}]} key={id}>
        {week}
        {selectWeekFunc &&
            selectWeekFunc(days)
        }
    ...
    const renderMonth = () => {
        ...
        return (
            <View>
                <View style={style.current.monthView}>{weeks}</View>
    ...
    const renderHeader = () => {
        ...
        return (<HeaderComponent {...headerProps} selectWeekFunc={selectWeekFunc} testID={`${testID}.header`} style={headerStyle} ref={ref} month={currentMonth} addMonth={addMonth} displayLoadingIndicator={shouldDisplayIndicator}/>);
    };
    const GestureComponent = false ? GestureRecognizer : View;
    const swipeProps = {
